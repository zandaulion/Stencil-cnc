...
**Implementation status (16 September 2026): the trust milestone through K08 is deployed, and K07 is implemented pending this deployment.** K01 remains the continuing regression workstream. The exact status and remaining tests are recorded below. Line references to audit evidence identify the original audited commit and must be rechecked as the code moves.
