...
**Implementation status (22 September 2026): the trust milestone through K11 is implemented. K12 guided exploration, K13 support interaction, the bounded K14 mobile/accessibility pass, K15 explainable repair planning, K16 responsive/cancelable processing, K17 draft export, and K18B guarded vector simplification are implemented; the K20 trusted-device surface is also implemented. K01's defined regression foundation is complete, and future packages must extend it with their own behavioral fixtures.** The exact status and remaining acceptance work are recorded below. Line references to audit evidence identify the original audited commit and must be rechecked as the code moves.
...
- **K01 — defined foundation complete, continuing by policy.** Deterministic storage, synchronization, geometry, candidate, export, and migration fixtures are joined by an isolated Playwright/Chromium harness. The real editor tests candidate restoration and geometry redraw, native editable-field undo without consuming project history, ruler values and zoom spacing, visibly watermarked draft PNG download, and semantic mobile-sheet focus/inert behavior. The fixture uses only generated masks, an in-memory server, temporary asset directories, and a network fence that rejects every external origin. Quota, physical suspension, multi-tab, CAM, and device checks remain acceptance work in their owning packages rather than unfinished K01 infrastructure.
...
- **K13 — bounded package implemented, app-level browser acceptance pending.** Pan is view-only and Support owns selection/editing. Supports have an accessible navigator with rationale, Select, Locate, Delete, previous/next controls, numeric centre/size/orientation fields, arrow movement, and separate-tap or drag creation. Smart planning now produces a dashed proposal with connectivity and fallback reporting; geometry changes only after explicit acceptance as one Undo step and one recovery point. Existing deterministic planner tests enforce feature preference without sacrificing connectivity or configured slat spans.
- **K14 — bounded package implemented; physical mobile and assistive-technology acceptance pending.** Narrow screens now keep a fixed canvas workspace and expose stage controls plus Candidates/Problems as compact bottom sheets. The permanent rail retains Projects, sync, Undo/Redo, and editing tools; Fit remains visible in a two-row canvas toolbar. Primary actions meet the measured normal-text contrast target, tab lists have roving keyboard behavior, visible focus is opaque, and result/error messages use stable live regions. Canvas gestures have one active primary-pointer owner and deterministic cancellation. Browser pinch zoom remains available in Pan, while editing modes suppress touch gestures only while they own the canvas. The explicit 320/390/430 px, landscape, larger-text, keyboard, Android/iOS, and screen-reader acceptance matrix is in `docs/K14_MOBILE_ACCESSIBILITY_ACCEPTANCE.md`; no blanket WCAG claim is made.
- **K09 — implemented, app-level browser acceptance pending.** Privacy, candidates, source storage, sharing, portable downloads, save acknowledgement, validation readiness, repair margins, and support fallbacks now describe their tested behavior. Geometry checks report blockers and advisory counts without claiming structural or process certification, and successful checks lead to CAM review rather than “ready to cut.” HEIC/HEIF is explicitly unsupported until a dependable browser-and-analysis decode path exists; the picker remains limited to PNG, JPEG, and WebP. Shared-project import no longer claims a server save while its upload is queued. A searchable, responsive in-app handbook now replaces ephemeral stage-help toasts and is mirrored by the comprehensive repository guide in `docs/KERFLOOM_USER_GUIDE.md`.
...
- **K15 — implemented, app-level browser acceptance pending.** Minimum-opening corrections are validated one region at a time against the complete blocker set. Safe local corrections survive unsafe neighbours, long/significant openings remain protected, and accepted automatic work stays active through continuous manual review. Every planning attempt—including a zero-change result—now persists an actionable report with added/removed/changed physical area, finished connectivity, full-width-core area/components, raster sampling uncertainty, proposal/accept/rejection counts, explicit planner budgets, grouped local rejection reasons, a structured stopping reason, and issue-specific next actions. The preview binds itself to the exact cutting-profile revision/status, dimensions, limits, geometry interpretation, and validation-model version; a changed contract cannot be applied, and accepted geometry is rechecked under the current matching contract. Diagnostics contain counts and reasons, not source imagery.
- **K16 — bounded package implemented; physical laptop/phone acceptance pending.** Validation, repair planning/evaluation, and smart-support planning run in a terminating module Worker with progress, explicit Cancel, latest-generation ownership, and revision/contract/signature checks before results are accepted. Server rendering retains abort semantics and exposes Cancel. Current geometry remains visible; cancelled, failed, superseded, or stale computations cannot enter Undo/save history. Failures remain visible with Retry/Dismiss, and a privacy-bounded handbook view records real per-device timings. The deterministic ARM64 host baseline measured 247.9 ms for a 249,480-cell A3 fixture and 250.3 ms for a 405,000-cell large-panel fixture; these justify off-thread work but are not physical-device or INP claims. See `docs/K16_PROCESSING_BASELINE.md`.
- **Nondestructive material-editing increment implemented; real-browser acceptance pending.** New Add, Remove, and Restore gestures are ordered physical-unit operations above the compatible legacy pixel layer. Strokes can be selected from the canvas or list, moved, resized, changed, hidden, and deleted without rewinding later work; region edits can be selected, changed, hidden, and deleted. Freehand smoothing, Shift angle constraint, brush-size shortcuts, Add/Remove swapping, temporary Space-pan, and an in-canvas physical-width HUD reduce tool travel. Project schema 6 and Candidate payload 4 preserve the complete operation layer while version-5 projects migrate without altering their existing painted cells. Deterministic unit/static tests cover replay order, Restore semantics, raster independence, migration, persistence, and interaction wiring; representative mouse, touch, long-stroke performance, and installed-PWA acceptance remain required.
...
- **K18B — implemented, target-CAM acceptance pending.** SVG/DXF contour simplification is explicitly optional and defaults off. Its tolerance is physical rather than decimal precision. Candidate polylines retain source vertices and must pass maximum-deviation, contour orientation, self/inter-contour intersection, nesting, and exact-circle checks. Kerfloom then rasterizes the actual vector output and re-runs the manufacturing validator; any new blocker or increased/new advisory automatically replaces the proposal with the unsimplified raster trace. The immutable release manifest hashes the actual vector set and records requested/applied tolerance, node counts, topology result, post-fit counts, and any fallback reason. This is straight-segment simplification, not curve fitting or machine certification.
...
- **K20 — trusted-device surface implemented; broader share ownership and recovery remain.** Projects now exposes a mobile-friendly Devices manager that creates same-workspace, one-time links with QR, copy, and native-share actions; lists last activity and pending invitations; cancels unused invitations; and disconnects other devices after explicit confirmation. Credentials use URL fragments so they are not included in HTTP request targets, and an already-linked browser must explicitly confirm replacing its workspace. Server routes scope every list, cancellation, and revocation to the authenticated workspace and prevent accidental self-revocation. Explicit sign-out/recovery and migration of per-device share ownership to the workspace remain open.

Latest verification: **287/287 JavaScript tests passed** and **82/82 Python tests passed**. The JavaScript count includes five real Chromium interactions for candidate restore, native field undo, physical rulers, draft PNG download, and mobile-sheet semantics. Syntax and diff checks passed. Variable Dots now preserve exact continuous circle primitives through placement, project/candidate persistence, server sync and sharing; preview/PNG are antialiased, and matched SVG/DXF openings export as arcs/`CIRCLE` entities rather than raster stair steps. The K18B suite also verifies physical-deviation bounds, topology rejection, prepared-geometry SVG/DXF output, circle rasterization, off-thread post-fit validation, release-vector hashes, and exact-raster fallback wiring. Client tests additionally verify ordered manual-edit replay and physical scaling, project/Candidate migration and persistence, editable-operation interaction wiring, gzip round-tripping, durable pending-count recovery, bounded/deduplicated retry policy, per-project failure isolation, damaged-outbox reconstruction, deterministic conflict retry, K09 truth-in-copy and handbook coverage, K10 shortcut/ruler behavior, K11 profile evidence/snapshot/migration behavior, K13 support navigation/proposal wiring and feature/span constraints, K14 primary-action contrast and tab/mobile/pointer wiring, K15 safe-subset repair, continuous manual review, conflict-scoped automatic-repair preservation, physical-effect/uncertainty reporting, explicit termination guidance, planner diagnostics, and profile/model binding, K16 latest-wins cancellation, structured-clone-safe background tasks, serialized feature guidance, and stale-result fences, plus K20 workspace-scoped device lifecycle authorization. HTTP integration verifies server-side decompression before encrypted storage. The existing `portfolio-screenshots/` assets were left untouched.
...
|---|---|---|
| 1. Open or resume a project | Projects, thumbnails, recovery points, last-project reopening. The Projects surface opens the device cache immediately, then refreshes server metadata without downloading every project. The active project is prioritized; another server-only project is downloaded when opened or explicitly saved for offline use. | Fast resumption should not require the entire library. K19 now provides cache-first navigation, metadata-first synchronization, active-project priority, and explicit offline retrieval. |
...
- [x] Add browser interaction tests for candidate restoration, editable-field undo, rulers, preview export, and later mobile changes. The Playwright harness drives the actual editor against an isolated local server rather than relying on HTML/source regular-expression assertions.
- [x] Keep fixtures synthetic or explicitly licensed; use semantic selectors and geometry assertions rather than brittle timing or pixel-only expectations. The harness builds generated masks, checks control/focus state and ruler geometry, verifies the downloaded PNG payload, and rejects external network requests.

**Status:** the defined K01 foundation is complete. `npm test` includes five headless Chromium interaction regressions; install its runtime once with `npm run test:browser:install`. Each later work package must still add the focused fixture needed for its own defect or behavior.
...
- [x] Add keyboard-accessible support selection/next/previous navigation and a concise list with select, locate, and delete.
- [x] Provide tap-start/tap-end creation and numeric/tap movement alternatives; retain unrestricted orientation, length, width, and undo.
- [x] Make Pan-versus-select behavior explicit and consistent; preserve dragging beyond canvas through pointer capture.
- [x] Preview smart-support suggestions with counts, affected areas, and fallback reasons. Accept as one undoable action.
- [x] Test feature preference against connectivity/span requirements; avoid moving a support into dark hair when it no longer supports the required geometry.

**Status:** bounded package implemented; app-level mouse, keyboard, and real-touch acceptance pending. Pan now moves only the view, while Support owns canvas selection and editing. Manual placement supports either one drag or separate start/end taps, with pointer capture retained for active gestures. Every applied support has a keyboard-accessible list entry with its placement rationale, Select, Locate, and Delete actions; P/N navigation, arrow movement, centre coordinates, unrestricted endpoint rotation, numeric length/angle/width, Delete, and Undo share the same geometry path. Smart planning draws a dashed proposal over the canvas and reports component-count change, support count, shortest-path fallbacks, and why fallbacks were needed. Existing geometry is unchanged until explicit acceptance, which commits the complete proposal as one undoable action and one recovery point. Existing deterministic regressions continue to require dark-feature placement to preserve connectivity and configured slat spans, including the sparse second-station case that routes around a bright face.
...
- [x] Fix primary-action contrast to at least 4.5:1 for normal text, including relevant interaction states.
- [x] Implement appropriate tab keyboard behavior, visible focus, labels, and persistent accessible result/error announcements.
- [x] Prototype canvas-first mobile adjustments with compact sheets; retain visible Projects, sync status, Undo/Redo, Fit, and a discoverable path to all stages.
- [x] Define active-pointer ownership, cancellation, and drawing-versus-navigation gestures. Do not disable browser accessibility zoom indiscriminately.
...
**Status:** bounded implementation complete; physical acceptance remains. White text on default and hovered primary-action fills measures 6.26:1 and 8.01:1 respectively, while the shared focus colour measures 7.61:1 against white. Narrow screens use a viewport-bound canvas with stage/review bottom sheets; selected stages remain reachable from both the workflow and an over-canvas Adjust action. Closed sheets are inert, focus enters their close action, and close/backdrop/Escape can restore the trigger. Workflow, review, and project-location tabs use a single tab stop plus Left/Right, Home, and End. Visual toasts are mirrored into stable polite/urgent live regions. Pan retains browser pinch zoom; geometry-editing modes claim only one primary pointer, ignore secondary editing pointers, use pointer capture, and release ownership on cancellation. The permanent bottom rail retains Projects, sync, Undo/Redo, and tools, while Fit stays visible in the canvas toolbar. Static/unit regressions cover these contracts. The separate acceptance matrix records 320/390/430 px, landscape, 200%/large text, keyboard, Android/iOS touch, and screen-reader checks as pending until run on representative browsers and devices. This is not a WCAG conformance declaration.
...
- [x] Keep blocking repair before optional warning reduction; retain preview/apply/discard and layer undo.
- [x] Report actual physical effects: surviving width/connectivity, affected area, changed artwork, and remaining uncertainty, not only segmented location counts.
- [x] Add clear termination reasons and relevant next actions: adjust size/density/profile, selected manual support, or review remaining advisories.
- [x] Track planner limits and rejected candidates for local diagnostics without logging photos.
- [x] Revalidate accepted geometry using the same versioned profile/model as export; a cosmetic score cannot override structural regressions.

**Status:** implemented; app-level browser acceptance pending. Minimum-opening repair no longer rolls back every useful correction because one neighbouring proposal is unsafe. Each local change must reduce its target without increasing another blocker, while significant openings and slat channels remain protected from destructive closure. Manual review is continuous and keeps accepted automatic work active. Planning now reports physical effect, connectivity and full-width-core survival, raster uncertainty, budgets, rejected candidates and reasons, a structured termination state, and issue-specific next actions. Zero-change attempts remain inspectable. The report captures the exact versioned profile/model contract; stale contracts fail closed before apply, and the resulting geometry is revalidated before export readiness. `Esc` clears problem focus without changing validation or geometry.
...
- [x] Add deterministic host fixtures plus local, privacy-bounded browser timing capture for rendering, validation, support planning, repairs, and saving. Physical Windows/phone measurements remain acceptance work.
- [x] Move demonstrated blocking geometry work to a worker with progress, termination-based cancellation, generation IDs, and stale-response rejection.
- [x] Retain old geometry until a new result is accepted; cancelled work cannot apply later or enter undo/save history.
- [x] Keep actionable failures beside the canvas until retried or dismissed; transient success toasts remain.
- [x] Coordinate render/source caching with K19 by introducing no second asset identity/cache system.

**Status:** bounded implementation complete; physical browser acceptance remains. The Worker owns validation, manufacturing-repair planning and choice re-evaluation, and smart-support graph planning. Results pass a latest-generation fence plus operation-specific revision/profile/signature checks. The server-render request remains abortable. A shared progress surface supports Cancel, Retry, Dismiss, elapsed time, stable live-region feedback, and Escape cancellation. Browser-local timing records contain no artwork. The deterministic ARM64 baseline and Windows/phone acceptance matrix are in `docs/K16_PROCESSING_BASELINE.md`.
...
- [x] Design optional polyline simplification using a physical error tolerance, preserving contour nesting and topology. Smooth curve fitting remains out of scope.
- [x] Validate actual output vectors after simplification and exact-circle substitution before export: direct topology/error checks plus rasterized manufacturing validation, with exact-raster fallback on regression.
...
**K18A status (2026-09-21):** implemented. The canvas and Export stage now report the actual physical X/Y cell size derived from the export mask and panel dimensions, together with an explicit raster-approximation warning. Every newly retained PNG, SVG, and DXF artefact carries a frozen version-1 release manifest. It binds the exact raster geometry and downloaded bytes with SHA-256 hashes and records sheet size, drawing units, working revision, exact-circle count, cutting-profile snapshot, finished-edge/CAM-compensation contract, exporter and validation-model versions, current validation time, blockers, advisories, filename, media type, and byte size. Manifests remain embedded in the append-only artefact record through encrypted project sync and sharing; the server rejects a manifest whose digest or output hash no longer matches. Legacy artefacts without a manifest remain readable.

**K18B status (2026-09-21):** implemented. Optional, default-off polyline simplification uses an operator-selected physical deviation and progressively lowers it when needed. Candidate vectors are rejected for excessive deviation, collapse/orientation change, self/inter-contour intersection, changed nesting, or invalid preserved-circle placement. The actual candidate vectors are rasterized at the export grid and revalidated against the active cutting profile; blockers and advisory regressions cause an automatic export of the original raster contours without circle substitution. SVG/DXF consume only this prepared geometry, while the release record hashes it and records model version, requested/applied tolerance, vertex reduction, direct checks, post-fit counts, and fallback reason. Straight-segment simplification reduces CAM node density but does not add information beyond the source grid. Known-dimension/feature import in the owner's CAM and physical coupon acceptance remain open.
...
- [x] Load metadata/thumbnails first and prioritize the active project; fetch other originals/exports on demand with an explicit offline-availability option. Server-only cards use metadata and saved thumbnails; opening downloads that project, while **Save offline** explicitly retains its complete source and exports.
- [x] Migrate existing bundles without discarding old versions until completeness and rollback are verified. A client first uploads and receives every immutable asset, then commits the manifest; the previous encrypted legacy bundle is retained as a tracked seven-day rollback revision. The compatibility route can hydrate manifests for older clients.
...
**K19 status (2026-09-22):** implemented. The browser keeps a durable complete queue snapshot for offline recovery, but online transfer resolves its source and exports to workspace-private immutable assets and sends only a small manifest on subsequent edits. Asset receipts are cached by digest inside the verified workspace database; stale receipts are discarded and uploaded again if the server rejects a reference. Server metadata is cached separately from editable projects, so library refresh downloads at most the active project (or the newest project on an empty device). Server-only cards can be opened on demand or explicitly made available offline. Legacy-to-manifest conversion retains the prior encrypted bundle for seven days, while reference-aware asset cleanup uses the same conservative grace window. Full portable bundles and project shares remain backward compatible.
...
- [x] Add a compact linked-device surface for naming, linking, last activity, pending-link cancellation, and revocation. Prevent current-device self-revocation and scope every action to the authenticated workspace.
- [ ] Add explicit sign-out and recovery paths without discarding unsynchronized local work.
...
Install the isolated Chromium runtime once with `npm run test:browser:install`. The audit baseline was 140 JavaScript tests and 71 Python tests. Test counts may increase; do not target those numbers by deleting coverage. If the virtual environment or browser dependencies are unavailable, report the missing prerequisite. Keep network-dependent setup and any sandbox approval separate from test assertions.
